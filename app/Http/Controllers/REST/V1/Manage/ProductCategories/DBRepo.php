...
class DBRepo extends BaseDBRepo
{
    ...
    {
        return !ProductCategory::where('name', $name)->where('outlet_id', $outletId)->exists();
    }

    public static function isNameUniqueOnUpdate(string $name, int $outletId, int $ignoreId): bool
    {
        return !ProductCategory::where('name', $name)->where('outlet_id', $outletId)->where('id', '!=', $ignoreId)->exists();
    }

    public static function findOutletId(int $categoryId): ?int
    {
        return ProductCategory::find($categoryId)?->outlet_id;
    }
}
